...
async function main() {
  ...
  console.log('=== check ===');
  let usdt_amount = await usdt.balanceOf(xbit.address);
  if (usdt_amount.lt(parseUnits('500', usdt_decimals))) {
    console.log('usdt pool is not full, no need to convert');
    return;
  }

  console.log('=== convert ===');
  // usdt_amount = parseUnits("10", usdt_decimals);
  tx = await xbit.transport(usdt_amount, tags);
  receipt = await tx.wait();
  let args_swap = receipt.events!.filter((x: Event) => x.event === 'TransportUSDT2WBTC')[0].args;
  let usdt_value = formatUnits(args_swap!.amount_usdt, usdt_decimals);
  let wbtc_value = formatUnits(args_swap!.amount_wbtc, wbtc_decimals);
