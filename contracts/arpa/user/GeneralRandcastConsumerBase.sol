    ...
    ConfirmedOwner
{
    ...
    function _requestRandomness(
        ...
        if (callbackGasLimit == 0) {
            uint256 gasUsed = _dryRunCallbackToEstimateGas(
                requestType,
                params,
                subId
            );
        ...
        if (requestConfirmations == 0) {
            (uint16 minimumRequestConfirmations, , , , , , ) = IAdapter(adapter)
                .getAdapterConfig();
            requestConfirmations = minimumRequestConfirmations;
        }
        return
            _rawRequestRandomness(
                requestType,
                params,
                subId,
    ...
    function _nextRequestId(uint64 subId) internal view returns (bytes32) {
        ...
        uint256 rawSeed = _makeRandcastInputSeed(
            _USER_SEED_PLACEHOLDER,
            subId,
            address(this),
            getNonce(subId)
    ...
    function _dryRunCallbackToEstimateGas(
        ...
        bytes memory params,
        uint64 subId
    ) internal isDryRun returns (uint256) {
        ...
        if (requestType == RequestType.Randomness) {
            ...
            for (uint256 i = 0; i < numWords; i++) {
                randomWords[i] = uint256(
                    keccak256(abi.encode(_RANDOMNESS_PLACEHOLDER, i))
                );
            }
            data = abi.encodeWithSelector(
        ...
        (
            bool success,
            bytes memory result // solhint-disable-next-line avoid-low-level-calls
        ) = address(this).call(
