...
import RandomnessProvider from './RandomnessProvider';
...
const PossibilityTable: React.FC<PossibilityTableProps> = ({
  ...
  const [randomnessProvider, setRandomnessProvider] =
    useState<RandomnessSource>(currentChainInfo().randomness);
  const handleSort = () => {
  ...
  return (
    <div
      className={`relative flex w-[378px] flex-col max-sm:mb-4 max-sm:w-full sm:ml-14 ${className}`}
    >
      <div className="flex items-center text-left max-sm:mb-4">
        <span className="mr-2 text-xl max-sm:text-lg">Prize Table</span>
        ...
        <div>
          ...
          <RandomnessProvider
            ...
            setValue={setRandomnessProvider}
          />
        </div>
      </div>
      <div className="overflow-y-auto sm:pt-8">
